...
public class Safen_cmd_queue {
	...
	public static void doMainProcess() {
		...
		String seq = "";
		/* 주문 정보 */
		Map<String, String> map_store_info = new HashMap<String, String>();
		if (con != null) {
			...
			try {
				...
				while(dao.rs().next()) {
					...
					map_store_info = getResultMapRows(dao.rs());
					...
					if (chk_seq) {
						...
						String hist_table = DBConn.isExistTableYYYYMM("sktl", "safen_cdr");
						int resultCnt2 = 0;
						...
						System.out.println("safen_in-> " + safen_in);
						
						if(safen_in.equals(""))
						{
							System.out.println("safen_in empty :  " + safen_in);
						...
						*/
						...
						/* 6. 적립조건 fivept
						 * 1. 캐시큐, 실버, 골드 가맹점 pre_pay == {"on","sl","gl", "br"}
						 * 2. 9 초 이상 통화
						 * 3. 이벤트 기간 이내
						 * 4. 금일 포인트 갯수는 0개 일때
						 * 5. 이벤트 갯수가 사용자가 가진 갯수를 초과 하지 않아야 한다.
						 * 6. 핸드폰인 경우
						 * 7. 파이브 포인트인 경우
						 * 8. 응답을 했을 경우
						 * 9. biz_code가 콜포인트와 일치하는 경우
						 * 10. 콜 포인트 인경우 
						 * 이상 위의 10가지의 조건을 만족하는 경우 파이브 포인트를 발급한다.
						 * 
						 * */
						
						/* 1. 캐시큐, 실버, 골드 가맹점 pre_pay == {"on","sl","gl", "br"} */
						call_point_condition = is_point(pre_pay);
						
						/* 2. 9 초 이상 통화 */
						call_point_condition = call_point_condition && service_sec>9;

						/* 3. 이벤트 기간 이내*/
						call_point_condition = call_point_condition && is_datepoint(ev_st_dt,ev_ed_dt);
							
						/* 6. 핸드폰인 경우 */
						call_point_condition = call_point_condition && is_hp;
						
						/* 7. 응답을 했을 경우  */
						call_point_condition = call_point_condition && is_answer;
						
						/* 8. biz_code가 콜포인트와 일치하는 경우 */
						call_point_condition = call_point_condition && chk_realcode;
						
						
						/* 위 6가지의 공통 조건을 만족 하는 경우*/
						five_point_condition = call_point_condition;
						
						/* 예외 조건, 4, 5, 10 */
						/* 4. 금일 포인트 갯수는 0개 일때  */
						five_point_condition = five_point_condition && calldaycnt==0;
						
						/* 5. 이벤트 갯수가 사용자가 가진 갯수를 초과 하지 않아야 한다.	 */
						five_point_condition = five_point_condition&& eventcnt<pt_event_cnt;
						
						/* 10. 파이브 포인트인 경우  */
						five_point_condition = five_point_condition &&is_fivept;
						...
						/* 6개의 공통 조건 */
						freedaily_point_condition = call_point_condition;
						
						/* 예외 조건 3가지, 4, 5, 10 */
						/* 4. 금일 포인트 갯수는 0개 일때  */
						freedaily_point_condition = freedaily_point_condition&&calldaycnt==0;
						
						/* 5. 이벤트 갯수가 사용자가 가진 갯수를 초과 하지 않아야 한다.	 */
						freedaily_point_condition = freedaily_point_condition&&eventcnt<pt_event_cnt;
						
						/* 10. 프리 데일리 포인트인 경우  */
						freedaily_point_condition = freedaily_point_condition&&is_freedailypt;
						
						
						/* freeuser_point_condition */
						/* 6개의 공통 조건 */
						freeuser_point_condition = call_point_condition;
						
						/* 예외 조건 3가지, 4, 5, 10 */
						/* 4. */
						freeuser_point_condition = freeuser_point_condition&&calldaycnt==0;
						
						/* 5. 이벤트 갯수가 사용자가 가진 갯수를 초과 하지 않아야 한다.	 */
						freeuser_point_condition = freeuser_point_condition&&eventcnt<pt_event_cnt;
						
						/* 10. 프리 유저 포인트인 경우 */
						freeuser_point_condition = freeuser_point_condition&&is_freeuserpt;
						
						/* review point 조건 */ 
						/* 6개의 공통 조건. */
						review_point_condition = call_point_condition;
		
						/* 예외 조건 2가지, 4,  10 
						 * 리뷰 포인트는 갯수 제한 없음
						 * */
						/* 4!. */
						review_point_condition = review_point_condition&&reviewdaycnt==0;
						
						/* 10!. */
						review_point_condition = review_point_condition&&is_reviewpt;
						
						
						/* down_point_condition */
						/* 6개의 공통 조건 */
						down_point_condition = call_point_condition;
						
						
						/* 예외 조건 2가지 4, 5, 10 
						 * */
						/* 4!. */
						down_point_condition = down_point_condition&&downdaycnt==0;
						
						/* 5!. */
						down_point_condition = down_point_condition&&eventcnt<pt_event_cnt;
												
						/* 10!. */
						down_point_condition = down_point_condition&&is_downpt;
						
						
						
						
						/* 1.  */
						sale_point_condition = call_point_condition;
						
						/* 4. */
						sale_point_condition =	sale_point_condition&&saledaycnt<pt_day_cnt;
						
						/* 5. */
						sale_point_condition =	sale_point_condition&&eventcnt<pt_event_cnt;
						...
						/* five edaily point 조건을 만족 했을 때 */
						}else if(freedaily_point_condition){
							/* freedailypt 7. 적립조건*/
							daily_st_dt=Utils.getyyyymmdd();
							daily_ed_dt=Utils.add60day();
						...
						}else if(freeuser_point_condition){
							/* freeuserpt 8. 적립조건*/
							/* 8-1 */
							set_0507_point(
								mb_hp,store_name, str_hangup_time, 
						...
						}else if(review_point_condition){
							/* reviewpt 9. 적립조건*/
							/* 9-1 */
							set_0507_point(
								mb_hp,store_name, str_hangup_time, 
...
*/
	...
	*/
	...
	private static String get_point(String pre_pay){
		String return_value = "0";
		switch (pre_pay) {
			case "gl":  return_value = "2000";
							 break;
			case "sl":  return_value = "2000";
							 break;
			case "on":  return_value = "1000";
							 break;
			case "br":  return_value = "500";
							 break;
	 ...
	 * @param String hist_table
	 * @param String calllog_rec_file
	 * @return
	 */
	public static int done_safen_cdr(String hist_table,String seq) 
	{
		boolean retVal = false;
		int last_id = 0;
		StringBuilder sb = new StringBuilder();
		...
		sb.append("insert into sktl.");
		sb.append(hist_table);
		sb.append(" select * from sktl.safen_cdr where seq=?");
		...
		try {
			...
			sb2.append("delete from sktl.safen_cdr where seq=?");
	
			dao2.openPstmt(sb2.toString());
			dao2.pstmt().setString(1, seq);
